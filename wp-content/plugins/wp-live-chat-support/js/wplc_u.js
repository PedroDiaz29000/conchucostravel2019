...
jQuery(document).ready(function() {
    ...
    if (typeof wplc_wpml_body_language !== "undefined") {
        ...
        Cookies.set('_icl_current_language', wplc_wpml_body_language, {
            expires: 1,
            path: '/'
        });
    }
...
function wplc_call_to_server_chat(data, first_run, short_poll) {
    if (typeof wplc_use_node_server !== "undefined" && wplc_use_node_server === "true") {
        ...
        if (typeof short_poll === "undefined") {
            short_poll = false;
        };
        ...
        wplc_server.send(wplc_ajaxurl, data, "POST", 120000, function(response) {
...
function wplc_loop_response_handler(response, data) {
    if (response) {
        ...
        data['action_2'] = "";
        if (typeof response['wplc_name'] !== "undefined") {
            data['wplc_name'] = response['wplc_name']; /* Cookies.set('wplc_name', response['wplc_name'], { expires: 1, path: '/' });*/
        }
        if (typeof response['wplc_email'] !== "undefined") {
            data['wplc_email'] = response['wplc_email']; /* Cookies.set('wplc_email', response['wplc_email'], { expires: 1, path: '/' }); */
        }
        if (typeof response['cid'] !== "undefined") {
            data['cid'] = response['cid'];
            Cookies.set('wplc_cid', response['cid'], {
                expires: 1,
                path: '/'
            });
        }
        if (typeof response['aname'] !== "undefined") {
            wplc_agent_name = response['aname'];
        }
        ...
        if (typeof response['status'] !== "undefined" && parseInt(wplc_chat_status) !== parseInt(response['status'])) {
            ...
            Cookies.set('wplc_chat_status', null, {
                path: '/'
            });
            Cookies.set('wplc_chat_status', wplc_chat_status, {
                expires: 1,
                path: '/'
            });
        }
...
(function($) {
    $(document).ready(function(event) {
        if (!window.wdtEmojiBundle) return;
        $(document.body).on("click", function(event) {
            // If click event isn't on the emoji window, or the emoji open button, close the emoji window
            if ($(event.target).closest(".wdt-emoji-picker, .wdt-emoji-popup").length == 0 && !(event.target.parentNode == null && $(event.target).hasClass("fa-smile-o"))) wdtEmojiBundle.close();
        });
