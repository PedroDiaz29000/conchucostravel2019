...
function Regitro_N($titulo,$detalle,$imagen){
	include('db.php');
		...
		$resul = mysql_query($consul,$cnx);		
		while($U_id=mysql_fetch_array($resul)){
			$varD = $U_id['id'];
		}

		$consul1='INSERT INTO detalle(noticia, rutafoto,orden,estado_foto)VALUES("'.$varD.'","'.$imagen.'",1,1)';
...
function insert_imagenes($id,$ruta){
		include('db.php');
	$date = date('Y/m/d h:i:s ', time());
		$resul='';
		$U_id='';
...
/*		$consul='INSERT INTO noticia(titulo,detalle,estado,fecha)VALUES("'.$titulo.'","'.$detalle.'",1,"'.$date.'")';
		...
		$consul='INSERT INTO detalle(noticia,rutafoto,orden,estado_foto)VALUES("'.$id.'","'.$ruta.'",2,1)';
		$resul = mysql_query($consul,$cnx);
//-- select last_insert_id();
		return $resul;
		mysql_close($cnx);
